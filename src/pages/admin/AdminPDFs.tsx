...
import type { PDF, Carrera } from '@/types';

const AdminPDFs: React.FC = () => {
  const { user } = useAuth();
  const { toast } = useToast();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [pdfs, setPDFs] = useState<PDF[]>(getPDFs());
  const [carreras] = useState<Carrera[]>(getCarreras());
  ...
  const filteredPDFs = useMemo(() => {
    return pdfs.filter(pdf => {
      const matchesSearch = pdf.nombre.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesCarrera = filterCarrera === 'all' || pdf.carrera === filterCarrera;
      return matchesSearch && matchesCarrera;
    });
  }, [pdfs, searchTerm, filterCarrera]);
  ...
  const handleSubmit = () => {
    ...
    if (!formData.carrera) {
      toast({
        title: 'Error',
        description: 'Debes seleccionar una carrera.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (selectedPDF) {
      ...
      const carreraNombre = carreras.find(c => c.id === formData.carrera)?.nombre || formData.carrera;
      
      const newPDF = createPDF({
        nombre: formData.nombre.trim(),
        carrera: formData.carrera,
        rolSubida: user.activeRole || 'admin',
      ...
      toast({
        title: 'Éxito',
        description: 'PDF subido correctamente. Los estudiantes de la carrera serán notificados.',
      });
      handleCloseDialog();
    }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Administra los documentos PDF por carrera</p>
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Lista de documentos PDF organizados por carrera</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {filteredPDFs.length === 0 ? (
              ...
              <div className="space-y-2">
                {filteredPDFs.map((pdf) => {
                  const carrera = carreras.find(c => c.id === pdf.carrera);
                  return (
                    <div
                      key={pdf.id}
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                {selectedPDF
                  ...
                  : 'Sube un archivo PDF y asócialo a una carrera.'}
              </DialogDescription>
            </DialogHeader>
